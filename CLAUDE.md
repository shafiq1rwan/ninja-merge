...
- **One column, one box**: the stage title bar, enemy status card, board and player HUD all use `BATTLE_LAYOUT.panel` (`data/battleAssets.ts`), which is derived from `BOARD_PIXEL_SIZE` and the board's x. Never hardcode a panel width in a battle scene - `tests/battleLayout.test.ts` asserts the panels match the board box, the margins are equal and the rows stack without overlapping. `TILE_SIZE`/`BOARD_GAP` also live there; `MergeTile`/`BoardView` re-export them for convenience.
